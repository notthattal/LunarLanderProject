...
def create_videos(baseFilename, env, q_network, num_vids, stepNum=0, fps=30):
    """
    Creates multiple videos of an agent interacting with a Gym environment.
    ...
    specified in the filename by providing a file extension (.mp4, .gif, etc..). 
    ...
    if stepNum == 0:
        pygame.display.set_mode((608, 400))

    for i in range(num_vids):
        filenameToUse = baseFilename
        
        if stepNum > 0:
            filenameToUse += "_AtStep_" + str(stepNum)
            if num_vids > 1:
                filenameToUse += "_Example" + str(i+1)
        else:
            filenameToUse += str(i+1) 
       
        filenameToUse += ".mp4"
        with imageio.get_writer(filenameToUse, fps=fps) as video:
            done = False
            state = env.reset()
...
def play_video(env, q_network, fps=30):
    done = False
    state = env.reset()
    frame = env.render(mode="rgb_array")
    screen = pygame.display.set_mode((608, 400))
    total_points = 0
    while not done:
        pygame_frame = convert_frame(frame)
    ...
    print("Total Points Earned: " + str(reward))

def convert_frame(frame):
    # Convert frame to Pygame surface
    frame = np.swapaxes(frame, 0, 1)  # Swap axes to match Pygame's shape expectations
    frame = frame[:, :, [2,1,0]]  # Convert RGB to BGR, which Pygame expects
